...
const ChatRoom = () => {
  ...
  const { username = "Guest", selectedLanguage = "en" } =
    (useLocation().state as { username: string; selectedLanguage: string }) ||
    {};
  const socketRef = useRef<Socket | null>(null);
  const localSocketIdRef = useRef<string | null>(null);
  const rtpCapabilitiesRef = useRef<mediasoup.types.RtpCapabilities | null>(
    null
  );
  const localDeviceRef = useRef<Device | null>(null);
  const localProducerTransportRef = useRef<mediasoup.types.Transport | null>(
    null
  );
  ...
  const localDataProducerRef = useRef<mediasoup.types.DataProducer | null>(
    null
  );
    ...
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        if (rtpCapabilitiesRef.current !== null) {
          await localDeviceRef.current?.load({
    ...
    return new Promise<void>((resolve) => {
      socketRef.current?.emit(
        ...
        (data: any) => {
          ...
          localProducerTransportRef.current =
            localDeviceRef.current?.createSendTransport(data) || null;
          console.log(
            "Local producer transport: ",
            localProducerTransportRef.current
          );

          localProducerTransportRef.current?.on(
            "connect",
  ...
  const produceVideo = async () => {
    localVideoProducerRef.current =
      (await localProducerTransportRef.current?.produce(videoParams)) || null;
  ...
  const produceAudio = async () => {
    localAudioProducerRef.current =
      (await localProducerTransportRef.current?.produce(audioParams)) || null;
      ...
      socketRef.current?.emit(
        ...
        (data: any) => {
          ...
          const consumerTransport =
            localDeviceRef.current?.createRecvTransport(data);
  ...
  const handleRemotePeer = async (peer: any) => {
    const { consumerTransport, serverReceiveTransportId } =
    ...
    for (const producer of peer.producers) {
      if (producer.kind === "audio" || producer.kind === "video") {
        const { consumer, id } = await createConsumer(
          consumerTransport,
          producer,
    ...
    setRemoteTrackGroups((prevTrackGroups: any) => {
      const updatedTrackGroups = [
        ...prevTrackGroups,
        { socketId: peer.socketId, tracks },
      ];
  ...
  const informPeers = () => {
    socketRef.current?.emit("new-peer", {
      roomId,
    ...
    socketRef.current?.on("peer-left", (socketId: any) => {
      ...
      setRemotePeers((prevPeers: any) => {
        const updatedPeers = prevPeers.filter(
          (peer: any) => peer.socketId !== socketId
        );
      ...
      setRemoteTrackGroups((prevTrackGroups: any) => {
        const updatedTrackGroups = prevTrackGroups.filter(
          (trackGroup: any) => trackGroup.socketId !== socketId
        );
        console.log(
          "Remote track groups updated: peer left: ",
          updatedTrackGroups
        );
        return updatedTrackGroups;
      });
  ...
  return (
    <div className="relative flex items-center justify-center h-screen bg-gray-900">
      ...
      <div className="relative z-10 flex w-[95%] h-[95vh] mx-auto p-6 bg-gray-800 bg-opacity-80 rounded-lg shadow-3xl overflow-hidden">
        ...
        <div
          className={`flex flex-col h-full bg-gray-700 p-4 rounded-lg shadow-lg transition-transform duration-300 ease-in-out ${
            isSidebarVisible ? "translate-x-0" : "translate-x-[-2000%]"
          }`}
        ...
        <div
          className={`flex flex-col ${
            isSidebarVisible ? "w-[90%]" : "w-full"
          } h-full space-y-4 bg-white-800 bg-opacity-80 overflow-hidden p-2 relative`}
        >
          ...
          {!isChatExpanded && (
            <div className="flex justify-center space-x-6 w-full p-4 border-gray-500 border-b-2">
              <video
                ...
                controls
                id="localVideo"
                className="w-[25%] h-[100%] border border-gray-600 rounded-md"
              />

              {remoteTrackGroups &&
                remoteTrackGroups.map((trackGroup: any, index: number) => {
                  const mediaStream = new MediaStream();
                  trackGroup.tracks.forEach((track: MediaStreamTrack) => {
                    mediaStream.addTrack(track);
                  });
                    ...
                    <video
                      ...
                      ref={(video) => {
                        if (video) {
                          ...
                          video.dataset.socketId = trackGroup.socketId;
                        }
                      }}
                      autoPlay
                      controls
          ...
          <div
            className={`w-full ${
              isChatExpanded ? "h-[95%]" : "h-[95%]"
            } p-4 rounded-lg overflow-y-auto relative`}
          >
            {messages.length === 0 ? (
              ...
              messages.map((msg, index) => (
                <div
                  ...
                  className={`flex items-center mb-3 ${
                    msg.socketId != localSocketIdRef.current
                      ? "justify-start"
                      : "justify-end"
                  }`}
                >
                  ...
                  >
                    ...
                    <span>
                      {translationEnabled && msg.translated
                        ? msg.translated
                        : msg.message}
                    </span>
                  </div>
